		...
		private void CastRayFromGaze () {
			...
			foreach (RaycastResult rayResult in m_RaycastResultCache) {
				...
				if (didHitValidComponent) {
					interactiveGazeEventDataList.Add (new InteractiveGazeEventData (InteractiveGazeEventType.HitInteractable, rayResult));	

				} else {						
					interactiveGazeEventDataList.Add (new InteractiveGazeEventData (InteractiveGazeEventType.HitCollider, rayResult));	

					resultsToRemove.Add (rayResult);
				}
			}
			if (m_RaycastResultCache == null || m_RaycastResultCache.Count == 0) {
				interactiveGazeEventDataList.Add (new InteractiveGazeEventData (InteractiveGazeEventType.HitNone, new RaycastResult ()));	
			}
