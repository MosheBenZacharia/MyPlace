...
namespace Utility 
{
	public class Threading {
		...
		public static void ExecuteJobOnSeperateThread<T,TResult>(Func<T, TResult> function, T param, Action<TResult> callback) {
			//Create Thread adapter
			ThreadAdapter adapter = (callback == null ? null : CreateThreadAdapter<TResult>());
			//Throw task into pool
			System.Threading.ThreadPool.QueueUserWorkItem(
				new System.Threading.WaitCallback(ExecuteJob<T,TResult>), (new object[] { adapter, function, param, callback })
			);
		}

		/// <summary>
		/// Converts between Action argument types - type safe?
		/// </summary>
		private static Action<object> Convert<TResult>(Action<TResult> myActionT)
		{
			if (myActionT == null) return null;
			else return new Action<object>(o => myActionT((TResult)o));
		}
		...
		private static void ExecuteJob<TResult>(object obj) {
			...
			Action<TResult> action = arr[2] as Action<TResult>;
			Action<object> callback = Convert<TResult> (action);
