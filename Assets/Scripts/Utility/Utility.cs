...
namespace Utility {
	public class Utility : MonoBehaviour {
		...
		public static Vector2 DirectionToCoordinates(Vector3 forward) {

			float x = Mathf.Atan2(forward.y, forward.x);
			float y = Mathf.Atan2(forward.z, Mathf.Sqrt(forward.x * forward.x + forward.y * forward.y));
		...
		public static int GetUnixTimestamp(DateTime localDateTime) {
			DateTime dateUTC = localDateTime.ToUniversalTime();
			return (int) (dateUTC.Subtract(new DateTime(1970, 1, 1).ToUniversalTime())).TotalSeconds;
		}

		public static int GetUnixTimestampNow() {
			return (int) (DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1).ToUniversalTime())).TotalSeconds;
		}

		// returns local DateTime 
		public static DateTime GetDateTimeFromUnixTimestamp(int timestamp) {
			// Unix timestamp is seconds past epoch
			DateTime dateTime = new DateTime(1970,1,1).ToUniversalTime();
